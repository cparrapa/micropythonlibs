from machine import Pin, PWM
import neopixel
import time
import random

# ========== CONFIG ==========
MATRIX_PIN = 18
WIDTH, HEIGHT = 8, 16
BRIGHTNESS = 0.3
LEFT_BTN_PIN = 27
RIGHT_BTN_PIN = 14
ENC_CLK = 21
ENC_DT = 22
BUZZER_PIN = 25
# ============================

# NeoPixel matrix
np = neopixel.NeoPixel(Pin(MATRIX_PIN), WIDTH * HEIGHT)

# Controls
btn_left = Pin(LEFT_BTN_PIN, Pin.IN, Pin.PULL_UP)
btn_right = Pin(RIGHT_BTN_PIN, Pin.IN, Pin.PULL_UP)
enc_clk = Pin(ENC_CLK, Pin.IN, Pin.PULL_UP)
enc_dt = Pin(ENC_DT, Pin.IN, Pin.PULL_UP)

# Buzzer setup
buzzer = PWM(Pin(BUZZER_PIN), freq=1000, duty=0)
def buzz(freq, dur=100, duty=200):
    buzzer.freq(freq)
    buzzer.duty(duty)
    time.sleep_ms(dur)
    buzzer.duty(0)
def blip(): buzz(1200, 40)
def click(): buzz(1500, 20)
def thud(): buzz(300, 60)
def jingle():
    for f in [600, 800, 1000, 1200]:
        buzz(f, 80)
        time.sleep_ms(30)
def sad_tone():
    for f in [400, 300, 200]:
        buzz(f, 200)
        time.sleep_ms(100)

# Fixed colors per shape
SHAPE_COLORS = [
    (255, 0, 0),     # O
    (0, 255, 0),     # T
    (0, 0, 255),     # I
    (255, 255, 0),   # S
    (0, 255, 255),   # Z
]

SHAPES = [
    # O
    [[(0,0),(1,0),(0,1),(1,1)]] * 4,

    # T
    [[(0,0),(1,0),(2,0),(1,1)],
     [(1,0),(1,1),(1,2),(0,1)],
     [(0,1),(1,1),(2,1),(1,0)],
     [(1,0),(1,1),(1,2),(2,1)]],

    # I
    [[(0,0),(1,0),(2,0),(3,0)],
     [(2,0),(2,1),(2,2),(2,3)],
     [(0,1),(1,1),(2,1),(3,1)],
     [(1,0),(1,1),(1,2),(1,3)]],

    # S
    [[(1,0),(2,0),(0,1),(1,1)],
     [(1,0),(1,1),(2,1),(2,2)],
     [(1,1),(2,1),(0,2),(1,2)],
     [(0,0),(0,1),(1,1),(1,2)]],

    # Z
    [[(0,0),(1,0),(1,1),(2,1)],
     [(2,0),(1,1),(2,1),(1,2)],
     [(0,1),(1,1),(1,2),(2,2)],
     [(1,0),(0,1),(1,1),(0,2)]],
]

# Game state
grid = [[(0,0,0) for _ in range(WIDTH)] for _ in range(HEIGHT)]
current_piece = None
px, py = 2, 0
rotation_index = 0

def scale_color(color):
    return tuple(int(c * BRIGHTNESS) for c in color)

def clear_matrix():
    for i in range(WIDTH * HEIGHT):
        np[i] = (0, 0, 0)

def draw_grid():
    for y in range(HEIGHT):
        for x in range(WIDTH):
            np[y * WIDTH + x] = scale_color(grid[y][x])

def draw_piece():
    shape = current_piece['shape']
    color = scale_color(current_piece['color'])
    for dx, dy in shape:
        x, y = px + dx, py + dy
        if 0 <= x < WIDTH and 0 <= y < HEIGHT:
            np[y * WIDTH + x] = color

def valid_position(x0, y0, shape):
    for dx, dy in shape:
        x, y = x0 + dx, y0 + dy
        if x < 0 or x >= WIDTH or y >= HEIGHT:
            return False
        if y >= 0 and grid[y][x] != (0, 0, 0):
            return False
    return True

def freeze_piece():
    for dx, dy in current_piece['shape']:
        x, y = px + dx, py + dy
        if 0 <= x < WIDTH and 0 <= y < HEIGHT:
            grid[y][x] = current_piece['color']

def remove_lines():
    global grid
    cleared = 0
    new_grid = []
    for row in grid:
        if all(p != (0,0,0) for p in row):
            cleared += 1
        else:
            new_grid.append(row)
    while len(new_grid) < HEIGHT:
        new_grid.insert(0, [(0, 0, 0)] * WIDTH)
    grid = new_grid
    if cleared:
        jingle()

def new_piece():
    global current_piece, px, py, rotation_index
    i = random.randint(0, len(SHAPES) - 1)
    rotations = SHAPES[i]
    color = SHAPE_COLORS[i]
    current_piece = {
        'rotations': rotations,
        'shape': rotations[0],
        'color': color
    }
    px, py, rotation_index = 2, 0, 0

def rotate_piece(clockwise=True):
    global rotation_index
    new_idx = (rotation_index + (1 if clockwise else -1)) % 4
    candidate = current_piece['rotations'][new_idx]
    if valid_position(px, py, candidate):
        current_piece['shape'] = candidate
        rotation_index = new_idx

def game_over():
    sad_tone()
    for _ in range(3):
        for i in range(WIDTH * HEIGHT):
            np[i] = scale_color((255, 0, 0))
        np.write()
        time.sleep(0.3)
        clear_matrix()
        np.write()
        time.sleep(0.3)
    reset_game()

def reset_game():
    global grid
    grid = [[(0, 0, 0) for _ in range(WIDTH)] for _ in range(HEIGHT)]
    new_piece()

# --- Game start ---
new_piece()
fall_timer = time.ticks_ms()
fall_interval = 500
move_timer = time.ticks_ms()
move_delay = 150

# Encoder state
last_clk = enc_clk.value()

# --- Main Loop ---
while True:
    clear_matrix()
    draw_grid()
    draw_piece()
    np.write()
    time.sleep_ms(1)

    # --- Encoder Rotation ---
    clk_val = enc_clk.value()
    if last_clk == 1 and clk_val == 0:  # Falling edge
        rotate_piece(clockwise=(enc_dt.value() == 1))
        click()
    last_clk = clk_val

    # --- Movement ---
    now = time.ticks_ms()
    if time.ticks_diff(now, move_timer) > move_delay:
        if not btn_left.value() and valid_position(px - 1, py, current_piece['shape']):
            px -= 1
            move_timer = now
            blip()
        elif not btn_right.value() and valid_position(px + 1, py, current_piece['shape']):
            px += 1
            move_timer = now
            blip()

    # --- Fall ---
    if time.ticks_diff(time.ticks_ms(), fall_timer) > fall_interval:
        fall_timer = time.ticks_ms()
        if valid_position(px, py + 1, current_piece['shape']):
            py += 1
        else:
            freeze_piece()
            thud()
            remove_lines()
            new_piece()
            if not valid_position(px, py, current_piece['shape']):
                game_over()
