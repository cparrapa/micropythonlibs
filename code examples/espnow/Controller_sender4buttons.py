...
while True:
    ...
    if current_buttonA_state != last_buttonA_state:
        ...
        current_buttonA_state = buttonA_pin.value()
        if current_buttonA_state == 1: # CHANGE TO 0 if button is Normally Open
            message = "A" # Press UP
        else:
            message = "AOff"
    ...
    if buttonA_pin.value() == 1 and buttonC_pin.value() == 1:
        message = "AC"
        print(f"Sending combo command: {message}")
        esp.send(peer, message.encode())
        utime.sleep_ms(300)  # Prevent multiple triggers

    elif buttonA_pin.value() == 1 and buttonD_pin.value() == 1:
        message = "AD"
        print(f"Sending combo command: {message}")
        esp.send(peer, message.encode())
        utime.sleep_ms(300)

    elif buttonB_pin.value() == 1 and buttonC_pin.value() == 1:
        message = "BC"
        print(f"Sending combo command: {message}")
        esp.send(peer, message.encode())
        utime.sleep_ms(300)

    elif buttonB_pin.value() == 1 and buttonD_pin.value() == 1:
        message = "BD"
        print(f"Sending combo command: {message}")
        esp.send(peer, message.encode())
        utime.sleep_ms(300)
        
    elif buttonA_pin.value() == 1 and buttonB_pin.value() == 1:
        message = "AB"
        print(f"Sending combo command: {message}")
        esp.send(peer, message.encode())
        utime.sleep_ms(300)
        
    elif buttonC_pin.value() == 1 and buttonD_pin.value() == 1:
        message = "CD"
        print(f"Sending combo command: {message}")
        esp.send(peer, message.encode())
        utime.sleep_ms(300)
