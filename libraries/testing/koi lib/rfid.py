...
class RFID:
    ...
    def __init__(self):
        try:
            ...
            screens.text('未检测到RFID模块！',5,5,1,(255, 255, 255))
            screens.text('请将RFID模块连接到右侧',5,30,1,(255, 255, 255))
    ...
    def _write(self, blockAddr, data):
        status = None
        backData = []
        backBits = None
    ...
    def MRFC522_read(self, address):
        ...
        self.i2cbus.writeto_then_readfrom(self.i2caddress,bytearray([address]),value)
        return value[0]

    def MRFC522_write(self, address, value):
        ##self.i2cbus.writeto_mem(self.i2caddress, address,bytearray([value]))
        self.i2cbus.writeto(self.i2caddress,bytearray([address,value]))
    ...
    def uuid(self,wait=3):
        try:
            self.MRFC522_init()
            startTime = time.monotonic()
            while True:
                if time.monotonic()-startTime > wait:
                ...
                if status == self.MI_OK:
                    ...
                    # Get UID of the card
                    (status, uid, backBits) = self.transceive()
                    if status == self.MI_OK:
                        uidStr = ""
                        for i in range(0,4):
    ...
    def read(self,blockAddr,wait=3,type="str"):
        ...
        while True:
            ...
            if status == self.MI_OK:
                ...
                if status == self.MI_OK:
                    ...
                    try:
                      (status, backData, backBits) = self.select(uid)
                    except:
                      return None
                    if status == self.MI_OK:
                        #print('Card selected')
                        ...
                        # Authenticate
                        mode = self.MIFARE_AUTHKEY1
                        (status, backData, backBits) = self.authenticate(
                            mode,
                            blockAddr,
                            self.MIFARE_KEY,
                            uid)
                        if (status == self.MI_OK):
                            ...
                            # Read data from card
                            (status, backData, backBits) = self._read(
                                blockAddr)
                            if (status == self.MI_OK):
                                if type=="bytes":
                    ...
                    if status == self.MI_OK:
                        ...
                        if (status == self.MI_OK):
                            ...
                            if (status == self.MI_OK):
                                ...
                                (status, backData, backBits) = self._write(
                                    blockAddr,
                                    dataASCLL)
                                if (status == self.MI_OK):
