...
button_a = Pin(BUTTON_PIN_A, Pin.IN, Pin.PULL_DOWN)
button_b = Pin(BUTTON_PIN_B, Pin.IN, Pin.PULL_DOWN)
...
while True:
    ...
    if button_presses != old_presses:
        print(button_presses)
        builtin_led.value(not builtin_led.value())  # Toggle the LED state
